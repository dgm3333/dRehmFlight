...
//========================================================================================================================//
// Uncomment this to switch between Teensy 4.0 or 4.1 motor/servo pinout
//#define TEENSY40
...
//#define USE_MPU9250_SPI         //The MPU9250 uses SPI connection for faster data transfer compared to I2C.
...
const int servo6Pin = 24;
const int servo7Pin = 25;
...
IMU Pin - Teensy 4.x Pin
VCC     - Any 5v Power
GND     - Any Ground
SCL     - 37 (SCK2)     // I2C or SPI Clock
SDA     - 35 (MOSI2)    // I2C or SPI Data Input
AD0/SDO - 34 (MISO2)    // SPI Data Output or I2C Slave Address configuration pin
NCS     - 36 (CS2)      // Chip Select for SPI mode only for MPU-9250
#endif
*/
...
//Radio comm:
// NB Despite the name these are used for SBUS as well as PWM hardware
...
float s1_command_scaled, s2_command_scaled, s3_command_scaled, s4_command_scaled, s5_command_scaled, s6_command_scaled, s7_command_scaled, s8_command_scaled, s9_command_scaled;
int s1_command_PWM, s2_command_PWM, s3_command_PWM, s4_command_PWM, s5_command_PWM, s6_command_PWM, s7_command_PWM, s8_command_PWM, s9_command_PWM;
...
#ifdef SERIAL_WEBSERVER
    Serial3.begin(WEBSERVER_SERIAL_SPEED);
    Serial.println("Connecting to dRehmFlight Webserver");
    setupWebserver();
#endif
...
#ifndef TEENSY40
    servo8.attach(servo6Pin, 900, 2100);
    servo9.attach(servo7Pin, 900, 2100);
#endif
...
void loop() {
    ...
    //Print data at 1M/SERIAL_OUTPUT_INTERVAL (default = 100hz)
    // (uncomment one at a time for troubleshooting) - SELECT ONE:
...
void IMUinit() {
     ...
     */
        ...
        if (status < 0) {
            ...
            Serial.println(" (see MPU9250.cpp for interpretation)"); 
            while(1) {}
        }
...
void setupWebserver() {
        ...
        for (int i=0; i<=dRehmVariableCnt; i++) {
                ...
                Serial3.print(dRehmVarType[i]);
                Serial3.print(dRehmVarName[i]);
                Serial3.print('=');
                if (dRehmVarType[i] =='i' or dRehmVarType[i] =='I') {
                        Serial3.print(*dRehmIntPtr[i]);
                } else if (dRehmVarType[i] =='f' or dRehmVarType[i] =='F') {
                        Serial3.print(*dRehmFloatPtr[i]);
                } else if (dRehmVarType[i] =='f' or dRehmVarType[i] =='F') {
                        Serial3.print(*dRehmStrPtr[i]);
                }
                Serial3.print('\n');                 // tell the webserver we're finished that variable
        }
        Serial3.print('X');                 // tell the webserver we're finished setup
}
...
void updateWebserverActions() {
        ...
        int incomingByte = Serial3.read(); // for incoming serial data
        ...
        while (incomingByte != -1 and endChr == FAILED_PACKET) {
                if (incomingByte == '\n')
                        endChr = NEW_PACKET;
                incomingByte = Serial3.read(); // for incoming serial data
        }
        ...
        if (actionType == 'G') {         //GET THE VARIABLE AND RETURN IT'S CURRENT VALUE VIA SERIAL
                Serial3.print(*dRehmFloatPtr[varID]);
        }
